...
export function StudentDetailModal({ studentId, isOpen, onClose }: StudentDetailModalProps) {
  ...
  useEffect(() => {
    console.log("StudentDetailModal useEffect:", { isOpen, studentId });
    if (isOpen && studentId) {
      console.log("모달이 열리고 학생 정보를 가져옵니다:", studentId);
  ...
  const fetchStudentDetail = async (id: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.message || "학생 정보를 불러오는데 실패했습니다.");
      }

      setStudent(data);
    } catch (err) {
      console.error("학생 상세 정보 조회 실패:", err);
      setError(err instanceof Error ? err.message : "학생 정보를 불러오는데 실패했습니다.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  console.log("모달 렌더링 중:", { isOpen, studentId, isLoading, error });
            ...
            <div className="relative bg-gradient-to-br from-[#8055e1] via-[#6f48d8] to-[#5b3ad6] px-8 pt-10 pb-14 text-white overflow-hidden">
              ...
              <div className="relative flex items-start gap-6">
                ...
                <div className="flex-1 pt-1">
                  <h2 className="text-3xl font-bold mb-3 tracking-tight">{student.name}</h2>
            ...
            <div className="p-6 space-y-3">
              ...
              <div className="relative bg-gradient-to-br from-[#f3edff] via-[#f0e9ff] to-[#ede6ff] rounded-xl p-4 shadow-lg border border-[#e5dbff]/50 hover:shadow-xl transition-shadow">
                ...
                <p className="text-xl font-bold text-[#8055e1] tracking-tight">
                  {formatPrice(student.preferred_price_min, student.preferred_price_max)}
                </p>
              </div>
              ...
              {student.goals && student.goals.length > 0 && (
                <div className="bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {convertArrayToStrings(student.goals, PURPOSE_ID_TO_NAME).map((goal, idx) => (
                      <span
                        key={idx}
              ...
              {student.subjects && student.subjects.length > 0 && (
                <div className="bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {convertArrayToStrings(student.subjects, SUBJECT_ID_TO_NAME).map((subject, idx) => (
                      <span
                        key={idx}
              ...
              {/* 수업 방식 */}
              {student.lesson_types && student.lesson_types.length > 0 && (
                <div className="bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-gray-600 mb-2 flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {convertArrayToStrings(student.lesson_types, LESSON_TYPE_ID_TO_NAME).map((type, idx) => (
                      <span
                        key={idx}
              ...
              {/* 지역 */}
              {student.regions && student.regions.length > 0 && (
                <div className="bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-gray-600 mb-2 flex items-center gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {student.regions.map((region, idx) => (
                      <span
                        key={idx}
              ...
              {student.created_at && (
                <div className="pt-3 border-t border-gray-200">
                  <div className="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <span>가입일: <span className="text-gray-700 font-medium">{formatDate(student.created_at)}</span></span>
                  </div>
                </div>
              )}
            </div>
